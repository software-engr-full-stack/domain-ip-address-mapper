...
run() {
  ...
  local ru="${PRIV_REMOTE_USER-}"
  if [ -z "$ru" ]; then
    ru="${TMPL_REMOTE_USER}"
  fi
  local remote_user="${ru:?ERROR => must provide PRIV_REMOTE_USER or TMPL_REMOTE_USER}"
...
setup_remote_user() {
  ...
  local rpw="${PRIV_REMOTE_PW-}"
  if [ -z "$rpw" ]; then
    rpw="${TMPL_REMOTE_PW}"
  fi

  local remote_pw="${rpw:?ERROR => must provide PRIV_REMOTE_PW or TMPL_REMOTE_PW}"
  ...
  echo "$remote_user:$remote_pw" | chpasswd
